import Head from "next/head";
import React, { useState } from "react";
import { FaCheckCircle, FaLocationArrow, FaMapMarkerAlt } from "react-icons/fa";
import CheckBox from "../ui/Checkbox";
// import Form from '../ui/Form';

export default function FindWork() {
  const [checked, setChecked] = useState(false);

  function handleChecked() {}

const nameData = [
  {
  name: "John Doe"
},
{
  name: "Jane Flona"
},
{
  name: "John Micheal"
},
{
  name: "Micheal Doe"
}
]

  return (
    <>
      <Head>
        <title>Find Work</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <section className="h-full">
      <header className="bg-black w-full h-full">
        <div className="grid grid-flow-row px-8 md:px-24 lg:items-center gap-4 lg:px-10 lg:grid-cols-12 py-32">
          <div className="col-span-5">
            <div>
              <div className="mb-8 lg:mb-28">
                <h1 className="text-white text-3xl lg:text-5xl font-bold">
                  Find Work
                </h1>
                <hr className="w-16 border-yellow-500" />
              </div>

              <div>
                <form className="h-10 bg-white w-full  lg:max-w-sm rounded-[2px] font-bold flex flex-row justify-between">
                  <input
                    className="h-10 bg-white w-full px-4 lg:pl-8 lg:max-w-sm rounded-[2px] outline-none border-none"
                    placeholder="search..."
                  />
                  <button className="bg-yellow-500 hover:bg-yellow-600 h-10 text-white px-4 max-w-xl rounded-r-[2px] font-bold">
                    Search
                  </button>
                </form>
              </div>
            </div>
          </div>
          <div className="col-span-7">
            <div className="max-w-md lg:block lg:max-w-xl">
              <h2 className="text-white text-3xl lg:text-4xl font-bold">
                Over <strong className="text-yellow-500">2000</strong> Jobs{" "}
                <br /> Posted daily
              </h2>
              <p className=" lg:block text-white text-md py-4">
                We make it easy for you to hire the most proficient Artisan with
                years of experience, and the exact skills you’re looking for.
              </p>
            </div>
            <div className="grid lg:grid-flow-col lg:items-center lg:gap-4">
              <form className="h-10 bg-white w-full lg:max-w-sm rounded-[2px] flex flex-row">
                <FaMapMarkerAlt className="text-black-400 lg:absolute text-base m-3" />
                <input
                  className="h-10 max-w-md  bg-white lg:px-10  lg:placeholder:text-sm rounded-md outline-none border-none"
                  placeholder="Enter your location"
                />
              </form>
              <select className="h-10 bg-white w-full rounded-[2px] lg:max-w-sm font-sm my-4 outline-none px-4">
                <option>All categories</option>
                <option>SW</option>
                <option>AS</option>
              </select>
              <select className="h-10 bg-white w-full rounded-[2px] lg:max-w-sm font-sm outline-none px-4">
                <option>All types</option>
                <option>ER</option>
                <option>EF</option>
              </select>
            </div>
          </div>
        </div>
      </header>
      <main className="grid grid-flow-row my-24 px-10 lg:grid-cols-12 gap-4 h-full">
        <div className="border-2 border-gray-400 rounded-lg w-full h-80 col-span-3">
          <div className="px-2 gap-4">
            <h3>Categories</h3>
            <div>
              <CheckBox label="Building maintence" onChange={handleChecked} />
              <CheckBox label="Painting" onChange={handleChecked} />
              <CheckBox label="Plumbing" onChange={handleChecked} />
              <CheckBox label="Carpenter" onChange={handleChecked} />
            </div>
            <div>
              <CheckBox label="Outdoors Services" onChange={handleChecked} />
              <CheckBox label="Gardening" onChange={handleChecked} />
              <CheckBox label="Massonry" onChange={handleChecked} />
              <CheckBox label="Surveying" onChange={handleChecked} />
            </div>
            <div>
              <p>Experience level</p>
              <CheckBox label="Beginner" onChange={handleChecked} />
              <CheckBox label="Intermediate" onChange={handleChecked} />
              <CheckBox label="Expert" onChange={handleChecked} />
            </div>
          </div>
        </div>
        <section className="border-2 border-gray-400 rounded-lg w-full h-full col-span-6 px-2">
          <section className="h-40 py-8 px-4 gap-4">
             <div className="bg-yellow-400 max-w-[130px] rounded-lg px-4">
              <p className="text-sm font-bold">Recommended</p>
            </div>
            {nameData.map((data) => (
            <section className="py-4 px-4">
              <section className="flex flex-row items-center justify-between">
                <div className="w-[80px] h-[80px] bg-gray-400 text-gray-400">
                  <p>000000000</p>
                  <p>000000000</p>
                </div>
                <div>
                  <h1 className="text-yellow-500 text-l font-bold">
                    {data.name}
                  </h1>
                  <p className="text-sm">Plumbing, Drainage management</p>
                </div>
                <div>
                  <p className="text-sm pb-2 text-gray-500 flex justify-end">
                    Hourly rate - ₦3000
                  </p>
                  <div className="bg-gray-200 max-w-[200px] rounded-lg px-4">
                    <p className="text-sm">Available 10 hours a week</p>
                  </div>
                </div>
              </section>
              <div className="w-full pt-2">
                <p className="text-sm">
                  I am looking for a Co-Founder to join me visualize an idea to
                  fruition. The Macro Idea is an Platform BYOB which stands for
                  BeYourOwnBoss will
                </p>
              </div>
             <section className="pt-4">
             <div className="bg-blue-600 max-w-[90px] justify-center text-white rounded-lg px-3 flex gap-2 items-center">
                <div><FaCheckCircle /></div>
                <p className="text-sm">Verified</p>
              </div>
              <div>
                {/* <Fag/> */}
              </div>
             </section>
            </section>))}
            <div>
              <hr className="w-full border-1 border-gray-400" />
            </div>
          </section>

          {/* <div className="">
            <button className="border-2 border-gray-400 px-4 py-[8px] font-bold rounded-lg">
              Load more
            </button>
          </div> */}
        </section>

        <div className="border-2 border-gray-400 rounded-lg w-full h-40 col-span-3">
          <div className="px-2 gap-4">
            <h3>Others</h3>
            <form>
              <input
                type="checkbox"
                name="Building maintence"
                value="Building maintence"
              />
              <input type="checkbox" name="Carpenter" value="Carpenter" />
              <input type="checkbox" name="Painting" value="Painting" />
              <input type="checkbox" name="Plumbing" value="Plumbing" />
            </form>
          </div>
        </div>
      </main>
      </section>
    </>
  );
}
